...
void strInsert(string &s) {
    vlong val0 = 0, val1 = 0;
    for (vlong i=0; i<s.size(); i++) {
        vlong p = (s[i] == 'R') ? 1 : 2;
        val0 = (val0 * 29 + p) % mod; 
        val1 = (val1 * 31 + p) % mod; 
        if (m.find(MP(val0,val1)) != m.end()) {
            return;
        }
    }
    m.insert(MP(val0,val1));
    res -= (1LL<<(n-s.size()));
    return;
}
...
int main () {
    ...
    freopen ( "Aoutput01.txt", "w", stdout );
    #endif // forthright48
