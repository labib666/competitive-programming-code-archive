...
4.  Pass highestNumberOfNode and
    flow (initial capacity of source) to maxFlow()
*/
...
struct FLOW {
    ...
    int vis[NODE], que[NODE], now[NODE];
    bool bfs ( ) {
    ...
    while (tc<t) {
        ...
        for (int i=1; i<=n; i++) {
            while (a[i] > 0) {
                dst[cur] = i;
                a[i]--;
                best = MAX(best,ABS(i-cur) + 1);
                for (int j=1,k=cur; k!=i; j++) {
                    if (k < i) {
                        res[j][k] = '\\';
                        k++;
                    }
