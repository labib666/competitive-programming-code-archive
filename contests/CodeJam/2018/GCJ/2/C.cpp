...
int main () {
    ...
    while (tc<t) {
        ...
        for (int k=0; k<(1<<m); k++) {
            if (POPCOUNT(k) >= best) continue;
            bool ok = 1;
            for (int i=0; i<n; i++) {
                for (int j=0; j<n; j++) {
                    if ( Check(k,id[i][j]) ) continue;
                    
                    for (auto u: kuhn.adj[id[i][j]]) {
                        if ( Check(k,u) ) continue;
                        ok = 0;
                        break;
                    }
            ...
            if (ok) best = MIN(best, POPCOUNT(k));
        }
