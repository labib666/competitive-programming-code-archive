...
int main (int argc, char **argv) {
    ...
    while (tc<t) {
        ...
        for (LL i=0; i<n; i++) {
            wt.push(make_data(w[i],i));
        }
        for (LL i=0; i<m; i++) {
            dt.push(make_data(0,i));
        }

        LL ans = 0, doneby, washby;
        for (LL i=0; i<l; i++) {
            data d1 = wt.top();
            wt.pop();
