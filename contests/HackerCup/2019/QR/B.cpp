...
bool solve(int pos, int bToRight) {
    ...
    int dist = n-1 - pos;
    int bToLeft = total - bToRight;

    if (dist >= lo && dist <= hi) {
        return true;
    } else {
        if (bToRight > 0 && solve(pos+2, bToRight-1)) {
            return true;
        }
        if (bToRight > 1 && solve(pos+3, bToRight-2)) {
            return true;
        }
        if (bToLeft > 0 && solve(pos-2, bToRight+1)) {
            return true;
        }
        if (bToLeft > 1 && solve(pos-3, bToRight+2)) {
            return true;
        }
        return false;
    }
