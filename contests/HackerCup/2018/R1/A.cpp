...
int t, tc;
vlong r = 3, c;
vlong dp[5][2000][2], seen[5][2000][2];
string s[3];

vlong solve(vlong x, vlong y, vlong st) {
    //debug(x,y,st);
    if (x < 0 || x >= r) return 0;
    if (y == c) {
        return (x==2) ? 1 : 0;
    ...
    dp[x][y][st] = 0;
    if (st == 1) {
        return dp[x][y][st] = solve(x,y+1,0);
    }
    else {
        for (vlong i=-1; i<=1; i++) {
            if (i == 0) continue;
            dp[x][y][st] = (dp[x][y][st] + solve(x+i,y,1)) % mod;
        }
    }
    return dp[x][y][st];
}
