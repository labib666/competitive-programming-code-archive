...
vlong t, tc = 0;
vlong n, m, mm, k, kk;
vlong C[1003][1003], D[1003], F[1003];
...
int main () {
    ...
    while (tc<t) {
		...
		for (vlong i=2; i<=1000; i++) {
			...
			D[i] = (D[i] + mm * D[i-1]) % mod;
		}
		
		//debug (n, m, k, mm, kk, "->", D[kk], F[mm], C[m][k]);
		
		vlong res = (D[kk] * F[mm]) % mod;
		res = (res * C[m][k]) % mod;
